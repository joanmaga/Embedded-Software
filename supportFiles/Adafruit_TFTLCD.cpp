...
// provide the underlying connections. It provides an API for the
// LCD controller.
...
//  CS_IDLE;  // BLH: CS is always asserted.
}
...
//        WR_STROBE; WR_STROBE; WR_STROBE; WR_STROBE; // x 4 pixels
    	  LCD_strobeWriteLine(); LCD_strobeWriteLine(); LCD_strobeWriteLine(); LCD_strobeWriteLine();
    	  LCD_strobeWriteLine(); LCD_strobeWriteLine(); LCD_strobeWriteLine(); LCD_strobeWriteLine();
      } while(--i);
    }
...
//      WR_STROBE;
//      WR_STROBE;
    	LCD_strobeWriteLine(); LCD_strobeWriteLine();
    }
  } else {
    while(blocks--) {
      i = 16; // 64 pixels/block / 4 pixels/pass
...
//      CD_COMMAND;
      LCD_setCommandMode();
      write8(0x00); write8(0x22); // Read data from GRAM
 //     CD_DATA;
      LCD_setDataMode();
...
//    CD_DATA;
    ...
    setWriteDir(); // Restore LCD data port(s) to WRITE configuration
//    CS_IDLE;
    return (((uint16_t)r & /*B11111000*/ 0xF8) << 8) |
           (((uint16_t)g & /*B11111100*/ 0xFC) << 3) |
           (           b              >> 3);
  } else return 0;
}
...
//  writeRegister8inline(a, d);
	// This functionality was copied from pin_magic.h which is not included or otherwise used here.
	LCD_setCommandMode();
	write8(a);
	LCD_setDataMode();
	write8(d);
...
//  writeRegister16inline(a, d);
	// From pin_magic.h
	uint8_t hi, lo; \
	hi = (a) >> 8; lo = (a); /*CD_COMMAND;*/ LCD_setCommandMode(); write8(hi); write8(lo);
	hi = (d) >> 8; lo = (d); /*CD_DATA*/ LCD_setDataMode();   ; write8(hi); write8(lo);
}
#endif
...
//  writeRegisterPairInline(aH, aL, d);
  // from pin_magic.h
  uint8_t hi = (d) >> 8, lo = (d); \
  /*CD_COMMAND;*/ LCD_setCommandMode(); write8(aH); /*CD_DATA;*/ LCD_setDataMode(); write8(hi);
  /*CD_COMMAND;*/ LCD_setCommandMode(); write8(aL); /*CD_DATA;*/ LCD_setDataMode(); write8(lo);
}
#endif
