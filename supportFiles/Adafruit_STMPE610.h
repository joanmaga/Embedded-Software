...
class TS_Point {
 public:
  ...
  bool operator!=(TS_Point);
  // BLH: maps the resistive point to LCD coordinates, only in portrait mode (tft.setRotation(1));
//  void mapToLcdCoordinates();
  int16_t x, y, z;
};
...
class Adafruit_STMPE610{
 public:
  ...
  void readData(int16_t *x, int16_t *y, uint8_t *z);
  uint16_t getVersion();
  bool touched(void);
  bool bufferEmpty(void);
  uint8_t bufferSize(void);
  TS_Point getPoint(void);
  void clearOldTouchData();  // Removes all current touch data from the FIFO.
